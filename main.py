'''The Random-Name-Generator takes in a list of names and a number
names wanted, then returns a list of random names.
'''
import random


def random_name_generator(shortlist_names, int_names):
    '''
    ...
    '''
    random_names = random.sample(shortlist_names, int_names)
...
if __name__ == '__main__':
    ...
    while True:
        ...
        try:
            num_names = int(input(f'You have {len(list_names)} names in your '
                                  'list. How many names do you want?\n'))
            if num_names <= 0:
